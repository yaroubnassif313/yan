...
def send_email(receiver_email, subject, body):
    """دالة عامة لإرسال الإيميلات للمستخدمين عبر حسابك"""
    msg = MIMEText(body)
    msg['Subject'] = subject
...
# --- 2. إدارة العمليات الحسابية والتحقق عبر قاعدة البيانات الرسمية ---
def check_college_records(university_id, full_name):
    """التحقق من وجود الطالب عبر مطابقة المدخلات مع جدول سجلات الكلية"""
    conn = sqlite3.connect('college_system.db')
    cursor = conn.cursor()
    # الكود هنا يقرأ مباشرة من الجدول الذي قمت بتغذيته ببياناتك ولا يحتوي على أي اسم مخفي
    ...
    conn.close()
    return result # يعيد بيانات القسم والكلية إذا تطابق الاسم والرقم، أو None إذا لم يوجدا

def add_user_account(email, university_id, password):
    """إنشاء حساب مستخدم جديد على موقع الويب بعد نجاح فحص الكلية"""
    conn = sqlite3.connect('college_system.db')
    cursor = conn.cursor()
    try:
        ...
        return False # الإيميل مسجل مسبقاً في الموقع
    finally:
        conn.close()

def verify_user_account(email):
    """تفعيل الحساب بعد إدخال الـ OTP الصحيح"""
    conn = sqlite3.connect('college_system.db')
    cursor = conn.cursor()
...
def check_login(email, password):
    """التحقق من بيانات تسجيل الدخول ومطابقتها"""
    conn = sqlite3.connect('college_system.db')
    cursor = conn.cursor()
    hashed_pass = hashlib.sha256(str.encode(password)).hexdigest()
    cursor.execute('''
...
def update_password(email, new_password):
    """تحديث كلمة المرور في قاعدة البيانات عند استعادتها"""
    conn = sqlite3.connect('college_system.db')
    cursor = conn.cursor()
...
# إدارة حالة التنقل بين الصفحات واحتفاظ البيانات المؤقتة
...
# --- الواجهة 2: إنشاء حساب جديد والتحقق الديناميكي من سجلات الكلية (Sign Up) ---
elif st.session_state.page == 'signup':
    st.title("📝 نموذج تسجيل طالب جديد")
    st.info("ملاحظة: يجب أن تتطابق المدخلات مع بياناتك الرسمية في سجلات الكلية.")
    
    student_name = st.text_input("الاسم الكامل (كما هو مسجل في الكلية):")
    uni_id = st.text_input("الرقم الجامعي الرسمى:")
    ...
    if st.button("التحقق وإرسال رمز الأمان 🚀", use_container_width=True):
        if student_pass != confirm_pass:
            ...
            # هنا يتم الاستعلام المباشر من ملف قاعدة البيانات المرفوع دون تدخل الكود
            ...
            if college_info:
                if add_user_account(student_email, uni_id, student_pass):
                    ...
                    st.error("❌ هذا البريد الإلكتروني مسجل مسبقاً بنظام الموقع!")
            else:
...
# --- الواجهة 5: تعيين كلمة المرور الجديدة بعد الرمز الصحيح ---
    ...
    if st.button("تعيين كلمة المرور الجديدة", use_container_width=True):
        if new_password == confirm_password:
            update_user_password(st.session_state.auth_data.get('email'), new_password)
            st.success("✅ تم تعيين كلمة المرور الجديدة بنجاح.")
            st.session_state.page = 'login'
            st.rerun()
        else:
            st.error("❌ كلمات المرور غير متطابقة، يرجى المحاولة مرة أخرى.")
