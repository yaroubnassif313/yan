...
# تنسيق مظهر المنصة الاحترافي ليطابق ملفك الأصلي تماماً
st.markdown("""
    <style>
...
# تم تثبيت كافة مفاتيح ملفك الأصلي بالإضافة لمفاتيح نظام التحقق والـ OTP لمنع أي انهيار
...
# --- 4. القائمة الجانبية الثابتة (شاشة الدعم الفني الكاملة) ---
# تم إصلاحها وتثبيتها لتظهر في كافة المراحل والصفحات للطلاب والمسؤول
with st.sidebar:
    ...
    # المرحلة الثانية: تسجيل الدخول
        ...
        if st.button("دخول آمن", type="primary"):
            if u_email and u_password:
                db = get_db()
                # مقارنة الباسورد المكتوب مشفراً لمطابقته مع الهاش المخزن لحماية الخصوصية
