...
# تنسيق CSS احترافي للمنصة (Modern UI)
st.markdown("""
    <style>
...
# --- 2. محرك قاعدة البيانات والوظائف الأمنية والربط ---
def get_db():
...
# تم تضمين كافة المفاتيح الأمنية والتنظيمية هنا لمنع أي AttributeError نهائياً
...
if st.session_state.step == 'welcome':
        ...
        if u_email and u_password:
            ...
            if user:
                if user['is_verified'] == 1:
                    ...
                    st.warning("⚠️ هذا الحساب معلق، يرجى إعادة تفعيله عبر التسجيل الجديد.")
            else:
    ...
    s_national = st.text_input("الرقم الوطني (11 رقم):")
    ...
    if st.button("التحقق والمصادقة الأمنية 🚀", type="primary"):
        if s_name and s_serial and s_national and s_email and s_pass:
            if not s_email.endswith("@gmail.com"):
                st.error("❌ يرجى إدخال بريد إلكتروني صحيح من نوع Gmail.")
