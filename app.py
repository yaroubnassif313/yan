    ...
    if st.button("التحقق والمصادقة الأمنية 🚀", type="primary"):
        if s_name and s_serial and s_national and s_email and s_pass:
            if not s_email.endswith("@gmail.com"):
                ...
                # تابع عملية التحقق
                st.success("✅ تم التحقق من البيانات بنجاح.")
                # هنا يمكنك إضافة الكود الخاص بعملية التحقق من الهوية وإرسال رمز التفعيل
                otp = str(random.randint(100000, 999999))
                st.session_state.auth_data['otp'] = otp
                st.session_state.auth_data['email'] = s_email
                ...
                if sent:
                    st.success(f"✅ تم إرسال الرمز بنجاح إلى {s_email}")
                else:
